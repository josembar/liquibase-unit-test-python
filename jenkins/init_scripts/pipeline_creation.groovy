...
def pipelineDefinition = [
	"liquibase-run-update": "Jenkinsfile-Update",
	"liquibase-run-rollback": "Jenkinsfile-Rollback",
	"database-unit-test": "Jenkinsfile-UnitTest"
]
def dir = "jenkins/pipeline_jobs/"
def flowDefinition, parent, job

for (element in pipelineDefinition) {
  	flowDefinition = new org.jenkinsci.plugins.workflow.cps.CpsScmFlowDefinition(scm, "${dir}${element.value}")
